...
export default function Preformance() {
    ...
    const fetchBit10Preformance = async (tokenPreformance: string) => {
        const response = await fetch(`api/${tokenPreformance}`);
        ...
        } else if (tokenPreformance === 'bit10-historic-data-meme-60') {
    ...
    const bit10Queries = useQueries({
        queries: [
            ...
            },
            {
                ...
                queryFn: () => fetchBit10Preformance('bit10-historic-data-meme-60')
            },
        ],
    });
